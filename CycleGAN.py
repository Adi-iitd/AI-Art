    ...
    with g_train.as_default():
        ...
        with tf.Session(graph = g_train) as sess:
            ...
            for iters in range(1, num_epochs*num_iters):
                ...
                _, D_A_Loss = sess.run([d_a_train_op, d_a_loss],     feed_dict = {input_a: img_a, input_b: img_b, train_mode: True, dropout: 0})
                _, G_A_loss = sess.run([g_b2a_train_op, g_b2a_loss], feed_dict = {input_a: img_a, input_b: img_b, train_mode: True, dropout: 0})

                _, D_B_Loss = sess.run([d_b_train_op, d_b_loss],     feed_dict = {input_a: img_a, input_b: img_b, train_mode: True, dropout: 0})
                _, G_B_Loss = sess.run([g_a2b_train_op, g_a2b_loss], feed_dict = {input_a: img_a, input_b: img_b, train_mode: True, dropout: 0})
                ...
                if iters%num_iters == 0:
                    ...
                    show_images(image_batch, tmp_path = "./Cycle_GAN_Output/", id = iters);
                
                    print('D_A_Loss: ' + str(tot_D_A_Loss/iters) + ', D_B_Loss: ' + str(tot_D_B_Loss/iters) + ', G_B2A_loss: ' + str(tot_G_A_loss/iters) \
