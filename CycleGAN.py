...
class ConcatenateDataset(Dataset):
    ...
    def __len__(self): 
        ...
        len_ = [len(dataset) for dataset in self.datasets]
        return min(len_)
...
trn_tfms  = [T.Resize(286), T.RandomCrop(256), T.RandomHorizontalFlip(), T.ToTensor(), normalize]
val_tfms  = [T.Resize(256), T.ToTensor(), normalize]
...
print(f"Total files (truncated to the length of smallest dataset) in the Train dataset: {len(trn_dataset)}")
...
plt.figure(figsize = (10, 5)); image_a, image_b = helper.get_random_sample(val_dataset)
image_a = helper.show_image(image_a, show = False); plt.subplot(1, 2, 1); plt.imshow(image_a)
image_b = helper.show_image(image_b, show = False); plt.subplot(1, 2, 2); plt.imshow(image_b)
...
class My_Conv(nn.Module):
    ...
    def __init__(self, in_channels: int = None, out_channels: int = None, kernel_size: int = 3, stride: int = 2, 
        ...
        Parameters: 
            ...
            padding_mode: Type of padding to use - 'Reflect' -OR- 'Zero' -OR- 'Replicate'
        """
        ...
        if  apply_nl:
            ...
            elif act_fn == 'lrelu': self.act_fn = nn.LeakyReLU(negative_slope = leak, inplace = False)
...
class My_DeConv(nn.Module):
    ...
    def __init__(self, in_channels: int = None, out_channels: int = None, kernel_size: int = 3, stride: int = 2, 
        ...
        if  apply_nl:
            self.act_fn = nn.ReLU(inplace = False); layers.append(self.act_fn)
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, apply_dp: bool = True, drop_param: float = 0.5, norm_type = 'instance',
        ...
        self.Conv_blk_1 = My_Conv(in_channels = in_channels, out_channels = in_channels, kernel_size = 3, stride =
                             1, norm_type = norm_type, act_fn = 'relu', padding = 1, padding_mode = padding_mode)
        ...
        self.Conv_blk_2 = My_Conv(in_channels = in_channels, out_channels = in_channels, kernel_size = 3, stride =
                             1, norm_type = norm_type, apply_nl = False, padding = 1, padding_mode = padding_mode)
...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 64, nb_resblks: int = 9, apply_dp: bool = True, 
                 drop_param: float = 0.5, norm_type: str = 'instance', padding_mode: str = 'reflect'):
        ...
        Parameters: 
            in_channels:    Number of input channels 
            out_channels:   Number of output channels
            nb_resblks:     Number of ResNet blocks to use in the Generator
            apply_dp:       If apply_dp is set to True, then activations are 0'ed out with prob "drop_param"
            norm_type:      Type of Normalization layer - InstanceNorm2D or BatchNorm2D
            padding_mode:   Type of padding to use - 'Reflect' -OR- 'Zero' -OR- 'Replicate'
        """
        ...
        conv = My_Conv(in_channels = in_channels, out_channels = out_channels, kernel_size = 7, stride = 1, 
                       leak = 0, norm_type = norm_type, act_fn = 'relu', padding = 3, padding_mode = padding_mode)
        self.layers.append(conv);
        
        
        nb_downsampling_layers = 2
        for i in range(nb_downsampling_layers):
            f = 2 ** i;
            conv = My_Conv(in_channels = out_channels * f, out_channels = out_channels * 2 * f, kernel_size = 3,
                      stride = 2, norm_type = norm_type, act_fn = 'relu', padding = 1, padding_mode = padding_mode)
            self.layers.append(conv)
        
        
        f = 2 ** nb_downsampling_layers
        for i in range(nb_resblks):
            res_blk = ResBlock(in_channels = out_channels * f, apply_dp = apply_dp, drop_param = drop_param, 
                               norm_type = norm_type, padding_mode = padding_mode)
            self.layers.append(res_blk)
        
        
        for i in range(nb_downsampling_layers):
            f = 2 ** (nb_downsampling_layers - i)
        ...
        conv = My_Conv(in_channels = out_channels, out_channels = in_channels, kernel_size = 7, stride = 1, 
                       apply_norm = False, apply_nl = False, padding = 3, padding_mode = padding_mode)
        self.layers.append(conv)
...
dis_A = init(Discriminator(in_channels = 3, out_channels = 64, nb_layers = 3))
dis_B = init(Discriminator(in_channels = 3, out_channels = 64, nb_layers = 3))

gen_A2B = init(Generator(in_channels = 3, out_channels = 64, nb_resblks = 9, padding_mode = 'reflect'))
...
class Losses:
    ...
    def _get_gen_cyc_loss(self, real_data, cycled_data):
        
        """
        Parameters:
            real_data:   Real images sampled from the dataloaders
            cycled_data: Image reconstructed after passing the real image through both the generators
                         X_recons = F * G (X_real), where F and G are the two generators
        """
        
        gen_cyc_loss = torch.nn.L1Loss()(real_data, cycled_data)
...
class ImagePool:
    ...
    """
    This class implements an image buffer that stores previously generated images! This buffer enables us to 
    update discriminators using a history of generated image rather than the latest ones produced by generator.
    """
    ...
    def sample_imgs_from_pool(self, images):
        
        """
        Parameters:
            images: the latest generated images from the generator
        
        Returns a batch of images from the pool.
        """
...
class CycleGAN:
    ...
    def write_summary(writer, gen, gen_loss: float, dis_loss: float, epoch: int, curr_iter: int, name: str):
        ...
        writer.add_scalar(name + '_gen_loss', round(gen_loss.item(), 3), n_iter)
        writer.add_scalar(name + '_dis_loss', round(dis_loss.item(), 3), n_iter)
    ...
    def _load_state_dict(self, path, train = True):
        ...
        if train:
            ...
            self.dis_A_optimizer.load_state_dict(checkpoint['d_A_opt'])
            self.dis_B_optimizer.load_state_dict(checkpoint['d_B_opt'])
            
            self.gen_A2B_optimizer.load_state_dict(checkpoint['g_A2B_opt'])
            self.gen_B2A_optimizer.load_state_dict(checkpoint['g_B2A_opt']) 
    ...
    def fit(self, nb_epochs: int = 1, dis_lr: float = 2e-4, gen_lr: float = 2e-4, beta_1: float = 0.5, beta_2:             float = 0.999, model_name: str = None, keep_only: int = 3, epoch_decay = 100):
        ...
        self.dis_A_optimizer = self._get_optimizer(self.dis_A.module.parameters(), lr = dis_lr, beta_1 = beta_1)
        self.dis_B_optimizer = self._get_optimizer(self.dis_B.module.parameters(), lr = dis_lr, beta_1 = beta_1)
        
        self.gen_A2B_optimizer = self._get_optimizer(self.gen_A2B.module.parameters(), lr = gen_lr, beta_1 = beta_1)
        self.gen_B2A_optimizer = self._get_optimizer(self.gen_B2A.module.parameters(), lr = gen_lr, beta_1 = beta_1)
        ...
        if self.lr_scheduler == 'linear':
            ...
            dis_A_scheduler = optim.lr_scheduler.LambdaLR(self.dis_A_optimizer, lr_lambda)
            dis_B_scheduler = optim.lr_scheduler.LambdaLR(self.dis_B_optimizer, lr_lambda)
            
            gen_A2B_scheduler = optim.lr_scheduler.LambdaLR(self.gen_A2B_optimizer, lr_lambda)
            gen_B2A_scheduler = optim.lr_scheduler.LambdaLR(self.gen_B2A_optimizer, lr_lambda)
        ...
        for epoch in range(start_epoch + 1, nb_epochs + 1):
            for (real_A, real_B) in trn_dataloader:
                ...
                gen_A2B_loss = tot_cyc_loss + gen_A2B_gan_loss + gen_A2B_idt_loss
                ...
                gen_B2A_loss = tot_cyc_loss + gen_B2A_gan_loss + gen_B2A_idt_loss
                
                self.gen_A2B_optimizer.zero_grad(); gen_A2B_loss.backward(retain_graph = True);
                self.gen_B2A_optimizer.zero_grad(); gen_B2A_loss.backward(retain_graph = False) 
                self.gen_A2B_optimizer.step(); self.gen_B2A_optimizer.step()
                
                
                # Discriminator's optimization step
                fake_A = self.fake_A_pool.sample_imgs_from_pool(fake_A)
                fake_B = self.fake_B_pool.sample_imgs_from_pool(fake_B)
                ...
                dis_A_loss.backward(); self.dis_A_optimizer.step()
                
                self.dis_B_optimizer.zero_grad() 
                dis_B_loss.backward(); self.dis_B_optimizer.step()
                
                
                # Write statistics to the Tensorboard
                if curr_iter % 50 == 0:
                    self.write_summary(self.writer, self.gen_A2B, gen_A2B_loss, dis_B_loss, epoch, curr_iter, 'A2B')
                    self.write_summary(self.writer, self.gen_B2A, gen_B2A_loss, dis_A_loss, epoch, curr_iter, 'B2A')
            ...
            if epoch % 10 == 0:
                ...
                torch.save({'epochs_': epoch, 
                            ...
                            'd_A_opt': self.dis_A_optimizer.state_dict(), 
                            'd_B_opt': self.dis_B_optimizer.state_dict(), 
                            
                            'g_A2B_model':  self.gen_A2B.module.state_dict(), 
                            'g_B2A_model':  self.gen_B2A.module.state_dict(), 
                            'g_A2B_opt': self.gen_A2B_optimizer.state_dict(), 
                            'g_B2A_opt': self.gen_B2A_optimizer.state_dict(),
