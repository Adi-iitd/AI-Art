...
class CustomDataset(Dataset):
    ...
    def __len__(self):
        ...
        return min(len(self.A_file_names), len(self.B_file_names))
...
class Tensorboard:
    ...
    @torch.no_grad()
    def write_loss(self, d_A_loss: float, d_B_loss: float, g_A2B_loss: float, g_B2A_loss: float, epoch: int, 
                   curr_iter: int):
        ...
        self.writer.add_scalar('g_A2B_loss', round(g_A2B_loss.item(), 4), n_iter)
        self.writer.add_scalar('g_B2A_loss', round(g_B2A_loss.item(), 4), n_iter)
        ...
        for epoch in range(start_epoch + 1, nb_epochs + 1):
            for data in trn_dataloader:
                ...
                if curr_iter % 10 == 0:
                    ...
                    self.tb.write_loss (d_A_loss, d_B_loss, g_A2B_loss, g_B2A_loss, epoch, curr_iter)
                
            curr_iter = 0; g_scheduler.step(); d_scheduler.step()
