...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        top_high = max(0, curr_height - self.image_size[0])
        lft_high = max(0, curr_width  - self.image_size[1])
        top = np.random.randint(low = 0, high = top_high)
        lft = np.random.randint(low = 0, high = lft_high)
...
class Pix2Pix:
    ...
    def fit(self, nb_epochs: int = 1, dis_lr: float = 2e-4, gen_lr: float = 2e-4, beta_1: float = 0.5, beta_2:             float = 0.999, root_dir: str = None, load_model: str = None, keep_only: int = 3, epoch_decay = 100):
        ...
        """
        Parameters: 
            root_dir:    Parent directory where models need to get saved 
            load_model:  Resume training from previous checkpoint with filename - "load_model"
            keep_only:   Number of models to keep in the root_dir
    ...
    def run_eval(self, root_dir: str = None, model_name: str = None):
        
        _ = self._load_state_dict(root_dir + model_name, train = False); 
        list_fake_labels = []; list_real_labels = []; list_real_images = []
        
        for idx, data in enumerate(val_dataloader):
            real_images, real_labels = data['image'].to(devices[0]), data['label'].to(devices[0])
            fake_labels = self.gen(real_images);  list_fake_labels.append(fake_labels);
            list_real_labels.append(real_labels); list_real_images.append(real_images);
        
        return list_fake_labels, list_real_labels, list_real_images
...
# fake_lab, real_lab, real_img = model.run_eval(root_dir = root_dir, model_name = "model_200.pth")
